<form method="post" action="<?php if(isset($BASE_URL)){ echo $BASE_URL; } ?>admin/categories/create/new" id="create_cat_form" style="width:100%">
    ...
    <div>
        ...
        <select name="fetch_pages[]"  multiple="multiple">
            <?php if(is_true_array($include_cats)){ foreach ($include_cats as $c){ ?>
               <option value="<?php   echo $c['id'];  ?>"> <?php for($i=0; $i <  $c['level'] ;$i++){?>-<?php } ?> <?php  echo $c['name'];  ?></option>
            <?php }} ?>
        </select>
        </div>
        <div class="clear"></div>
...
<?php $mabilis_ttl=1309329751; $mabilis_last_modified=1289912984; //X:\home\kupontut.il\www\/templates/administrator/create_cat.tpl ?>